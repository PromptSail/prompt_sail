...
def test_transaction_openai_ok(client, application, monkeypatch):
    ...
    # act
    resp = client.post(
        "/project1/openai/chat/completions", {"model": "gpt-3.5-turbo", "messages": [
                {
                    "role": "system",
                ...
                {
                    "role": "user",
                    "content": "Name the elements of human body."
                }
            ]
        }
    )
...
def test_transaction_openai_unauthorized(client, application, monkeypatch):
    ...
    assert resp.json()["error"]["message"] == "You didn't provide an API key. You need to provide your API key in an Authorization header using Bearer auth (i.e. Authorization: Bearer YOUR_KEY), or as the password field (with blank username) if you're accessing the API from your browser and are prompted for a username and password. You can obtain an API key from https://platform.openai.com/account/api-keys."
    ...
    resp = client.post(
        "/project1/azure/openai/deployments/gpt-35-turbo-test/chat/completions", {
            ...
            "messages": [
              {
                "role": "user",
                "content": "What to do to live good life."
              }
            ]
        }
    )
...
def test_transaction_azure_unauthorized(client, application, monkeypatch):
    ...
    # assert
    assert resp.status_code == 401
    assert resp.json()["message"] == "Unauthorized. Access token is missing, invalid, audience is incorrect (https://cognitiveservices.azure.com), or have expired."
